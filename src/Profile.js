...
export default function Profile({ darkMode }) {
  ...
  useEffect(() => {
    const unsubscribe = auth?.onAuthStateChanged(async (currentUser) => {
      if (currentUser) {
        setUser(currentUser);
        setCredits(getCredits(currentUser.uid) || 0);
        ...
        try {
          ...
          console.error("Error loading profile from Firebase, using localStorage:", error);
          const localData = loadFromLocalStorage();
          setDisplayName(localData.displayName);
          setPhotoURL(localData.photoURL);
        }
      ...
      // If auth is not available, redirect to home
      navigate("/");
      setLoading(false);
    });
        ...
        <div className={`rounded-xl shadow-lg p-6 sm:p-8 ${
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className={`px-4 py-3 rounded-lg ${
                darkMode ? 'bg-gray-700 text-green-400' : 'bg-gray-50 text-green-600'
