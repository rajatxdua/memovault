...
import { getCredits, getRemainingFreeChats, shouldChargeCredits, deductCredits } from "./utils/credits";
...
export default function Home({ showThemes, setShowThemes, darkMode: appDarkMode, user: appUser }) {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setCreditsState(getCredits(storedUser.uid) || 0);
      setFreeChatsRemaining(getRemainingFreeChats(storedUser.uid));
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((currentUser) => {
      ...
      if (currentUser) {
        ...
        setCreditsState(getCredits(currentUser.uid) || 0);
        setFreeChatsRemaining(getRemainingFreeChats(currentUser.uid));
  ...
  const login = async () => {
    try {
      ...
      setCreditsState(getCredits(result.user.uid) || 0);
      setFreeChatsRemaining(getRemainingFreeChats(result.user.uid));
  ...
  const sendMessage = async () => {
    ...
    try {
      // Check if we need to charge credits or use free quota
      const needToChargeCredits = shouldChargeCredits(user.uid);
      
      if (needToChargeCredits) {
        // Check if user has enough credits (cost: 10 credits per message after free limit)
        const currentCredits = getCredits(user.uid);
        if (currentCredits < 10) {
          setAiTyping(false);
          alert("You've used your 100 free daily messages! You need 10 credits for additional messages. Get more credits!");
          setShowCredits(true);
          return;
        }

        // Deduct credits for AI chat
        if (!deductCredits(user.uid, 10)) {
          setAiTyping(false);
          ...
          {!user ? (
            <div className="flex flex-col items-center justify-center text-center w-full space-y-4 sm:space-y-6">
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <p className="text-xs sm:text-sm md:text-base lg:text-lg text-gray-600 dark:text-gray-300 max-w-xs sm:max-w-sm md:max-w-md mx-auto leading-relaxed px-2">
                  Your AI-powered memory companion. Start with 100 free daily conversations with Gemini-powered AI.
                </p>
              </div>
            ...
            <div className="w-full flex flex-col h-[60vh] sm:h-[65vh] md:h-[70vh] lg:h-[75vh] xl:h-[80vh] min-h-[500px] max-h-[800px]">
              ...
              <div className={`flex justify-between items-center mb-3 p-3 rounded-lg ${
                ...
                <div className="flex items-center space-x-2">
                  {/* Free chats remaining indicator */}
                  {freeChatsRemaining > 0 ? (
                    <span 
                      className="bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-100 px-2 py-1 rounded-lg font-mono font-bold text-xs cursor-pointer hover:bg-blue-200 dark:hover:bg-blue-700 transition-colors"
                    ...
                    <span 
                      className="bg-orange-100 dark:bg-orange-800 text-orange-800 dark:text-orange-100 px-2 py-1 rounded-lg font-mono font-bold text-xs"
                  ...
                  {/* Credits */}
                  <span 
                    className="bg-green-100 dark:bg-green-800 text-green-800 dark:text-green-100 px-2 py-1 rounded-lg font-mono font-bold text-xs cursor-pointer hover:bg-green-200 dark:hover:bg-green-700 transition-colors"
